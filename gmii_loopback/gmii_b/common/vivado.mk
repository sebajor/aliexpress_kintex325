...
# Vivado project file
$(FPGA_TOP).xpr: Makefile $(XCI_FILES_REL)
	rm -rf defines.v
	touch defines.v
...
# synthesis run
$(FPGA_TOP).runs/synth_1/$(FPGA_TOP).dcp: $(FPGA_TOP).xpr $(SYN_FILES_REL) $(INC_FILES_REL) $(XDC_FILES_REL)
...
# implementation run
$(FPGA_TOP).runs/impl_1/$(FPGA_TOP)_routed.dcp: $(FPGA_TOP).runs/synth_1/$(FPGA_TOP).dcp
...
# bit file
$(FPGA_TOP).bit: $(FPGA_TOP).runs/impl_1/$(FPGA_TOP)_routed.dcp
	...
	mkdir -p rev
	EXT=bit; COUNT=100; \
	while [ -e rev/$(FPGA_TOP)_rev$$COUNT.$$EXT ]; \
